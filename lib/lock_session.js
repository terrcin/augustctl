...
function LockSession(peripheral, writeHandle, readHandle, indicationHandle) {
  this._peripheral = peripheral;
  this._writeHandle = writeHandle;
  this._readHandle = readHandle;
  this._indicationHandle = indicationHandle;
  return this;
}
...
var ENABLE_INDICATIONS = new Buffer('0200', 'hex');

LockSession.prototype.start = function() {
  // decrypt all reads, modifying the buffer in place
  this._peripheral.on('handleNotify', function(handle, data) {
    // only process if it's associated with this session
    if (handle !== this._readHandle) {
      return;
    }
  ...
  // enable indications
  debug('enabling indications on ' + this._indicationHandle);
  return this._peripheral.writeHandleAsync(this._indicationHandle, ENABLE_INDICATIONS, false);
};
